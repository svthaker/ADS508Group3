...
def load_master_dataset() -> pd.DataFrame:
    """
    Merge all five cleaned sources on (county, state).
 
    Sources expected in PROCESSED_DIR:
        food_access_county_clean.csv        — target + food access features
        census_features_clean.csv           — demographics
        places_ca_health_clean.csv          — health outcomes (CA only)
        food_environment_clean.csv          — env poverty/diabetes/obesity
        california_food_store_locations.csv — point-level stores (aggregated here)
    """
...
if "county" in stores_raw.columns:
   ...
   print(
            ...
            "Skipping store density features. Run a spatial join first."
        )
     store_agg = None
...
def _norm(df: pd.DataFrame) -> pd.DataFrame:
    ...
    food_access = _norm(food_access)
    census      = _norm(census)
    food_env    = _norm(food_env)
 
# PLACES uses state abbreviation "ca" → normalise to "california"
...
master = food_access.merge(
        census.drop(columns=["geo_id", "county_name", "male_population",
                              "female_population"], errors="ignore"),
    ...
    if store_agg is not None:
        master = master.merge(store_agg, on="county", how="left")
...
#Split CSVs
    for split_name, X_split, y_split in [
        ...
        _upload_to_s3(str(path), f"processedData/{split_name}_split.csv")
 
    #Scaler
    scaler_path = MODELS_DIR / "scaler.joblib"
    joblib.dump(scaler, scaler_path)
    _upload_to_s3(str(scaler_path), "models/scaler.joblib")
 
    #Model
    model_path = MODELS_DIR / "random_forest_baseline.joblib"
    joblib.dump(rf, model_path)
    _upload_to_s3(str(model_path), "models/random_forest_baseline.joblib")
    ...
    _upload_to_s3(str(metrics_path), "models/metrics.json")
 
    #Feature importance
    imp_path = MODELS_DIR / "feature_importances.csv"
    importance_df.to_csv(imp_path, index=False)
    _upload_to_s3(str(imp_path), "models/feature_importances.csv")
